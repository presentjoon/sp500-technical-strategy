...
> 기존 변동성 정보의 재표현인지는 **[D8 층화 반증 테스트 결과]** 로 판별한다.

**앞 세 문장은 D8 결과와 무관하게 확정되어 있다.** 대괄호 부분만 D8 층화
반증 테스트 후에 채운다. 사전등록은 `docs/prereg_d08_stratified.md`.

> **D8과 D12는 다른 작업이다.** D8은 **사후 실현변동성**을 종속변수로 두는
...
만드는 것을 막기 위해서다. D8 결과가 어느 쪽으로 나오든 앞 세 문장은
...
| 항목 | 사유 | 이월처 |
|---|---|---|
| **층화 반증 테스트** (직전 14일 실현변동성 5분위 내 비교, 종속변수 = **사후 실현변동성**) | 2.14배가 RSI 고유 정보인지 직전 변동성의 재표현인지 판별. **p값 없음 → $m$ 무관** | **D8** |
...
| 일자 | 작업 |
|---|---|
| **D8** | **층화 반증 테스트 (변동성)** — 사전등록 `docs/prereg_d08_stratified.md` |
