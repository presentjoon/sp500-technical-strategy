...
S1/h=1의 최초 유의성은 견고한 평균수익 효과로 해석할 수 없었다. 사전등록 순열검정에서 초과수익은 S1의 롱·평균회귀 가설과 반대인 **−0.502%p**였고, 평균을 검정통계량으로 사용했을 때 $p=0.0019$로 Bonferroni 기준 **$\alpha_{adj}=0.0025$**를 통과했다. 그러나 스튜던트화 통계량으로 같은 자료를 다시 검정하면 $p=0.1244$로 상승하여 유의성이 사라진다.

여기서 $p=0.0019 \rightarrow 0.1244$의 변화는 데이터가 바뀌어서 발생한 것이 아니다. **같은 관측값에 대해 검정통계량을 평균 차이에서 분산 차이에 강건한 스튜던트화 통계량으로 바꾸었기 때문에 발생한 변화다.** 따라서 최초의 작은 $p$값 자체보다, 어떤 검정통계량이 그 값을 만들어냈는지를 먼저 확인해야 했다.

그 원인은 S1 신호일의 수익률 산포가 대조군과 크게 달랐다는 데 있었다. S1/h=1의 `sd_ratio`는 **2.1437**로 20개 조합 중 가장 컸다. 즉 S1 신호일의 익일 수익률 표준편차가 대조군의 약 **2.14배**였다. 여기서 대조군은 전 거래일 전체이며, 신호일 65건도 분모에 포함된다. 이처럼 작은 집단의 분산이 크게 다른 상황에서 평균만을 이용한 순열검정은 평균이 동일하다는 약한 귀무가설을 검정하는 데 적합하지 않으며, Behrens–Fisher 상황에서는 작은 집단의 분산이 클 때 반보수적(anti-conservative)이 될 수 있다. S1/h=1에서는 바로 이 조건이 나타났고, 그 결과 평균 통계량의 $p=0.0019$가 실제 평균 차이에 대한 근거보다 작게 나타날 가능성이 있었다.

스튜던트화하면 이 분산 불균형의 영향을 반영하여 검정통계량의 절대값이 작아지고, $p$는 **0.1244**로 상승한다. 따라서 이 1건은 "S1의 익일 평균수익률이 유의하게 낮다"는 발견으로 남는 것이 아니라, **평균 차이에 기반한 최초의 유의성이 이분산에 민감한 검정 방식에서 발생한 것임을 보여주는 사례**로 정리하는 것이 타당하다.

이 사건은 이후 분석의 방향도 바꾸었다. D7에서 확인된 것은 단순히 "유의한 수익률 효과가 사라졌다"는 사실만이 아니라, **신호일의 산포가 크게 다르다는 사실 자체가 검정 결과를 좌우할 수 있다는 점**이었다. 이 관측에서 출발하여 D8b에서는 평균수익률이 아니라 사건별 사후 경로변동성을 별도의 종속변수로 정의하고, 신호일의 높은 산포가 어떤 형태의 변동성 정보와 관련되는지를 검토하게 되었다. 따라서 D8b는 S1/h=1의 유의성을 되살리기 위한 후속 검정이 아니라, D7에서 관측된 산포 차이의 성격을 별도의 종속변수로 확인하기 위해 출발한 분석이다.

> D7과 D8b 사이에 시도되었다가 종속변수 정의 문제로 폐기된 D8 층화 설계에 대해서는 `reports/why_it_fails.md` §3 참조.
...
## 6. 익일 수익률 산포 2.14배 — 관측과 그 한계

S1/h=1에서 최초 결과와 함께 확인된 중요한 관측은 평균의 방향이 아니라 **수익률 산포의 크기**였다. S1 신호일의 익일 수익률 산포는 대조군보다 **2.1437배**였다. 이는 S1 신호가 발생한 날의 다음 거래일 수익률이 평균적으로 어느 한 방향으로 움직였다는 것과는 다른 현상이다. 같은 평균이라도 결과의 산포가 크다면 개별 사건의 실현 결과가 서로 크게 벌어질 수 있으므로, 이 관측은 S1 신호와 수익률의 관계를 평균 하나만으로 설명하기 어렵다는 점을 시사한다.

3일의 극단 관측을 제거하면 `sd_ratio`가 **2.1437에서 1.4949**로 감소하여 사전등록한 **1.5 기준에 미달**했다. 같은 조건에서 평균 차이도 약 **5.0배 축소**되었다. 따라서 이 산포 차이를 이상치에 강건한 발견으로 확정하지 않았으며, 분산비 역시 강건한 발견으로 판정하지 않았다.

> `sd_ratio_full` / `sd_ratio_kept`(`src/diagnostics.py` 492–493행)의 분모 `pool_sd`가 본문 `sd_ratio`의 분모 `sd_universe`(133행)와 동일한지는 D15 시점 미확인. 2단계에서 확인 예정.

특히 여기서 D7의 `sd_ratio`와 이후 D8b에서 측정한 경로변동성을 동일한 개념으로 취급해서는 안 된다. D7의 2.1437은 **여러 사건 시점에서 관측된 익일 수익률들의 산포 비율**이다. 반면 D8b는 사건별 20일 경로에서 계산한 변동성을 비교하도록 새롭게 정의한 지표다. 따라서 D7의 산포 관측만으로 "신호가 이후 변동성이 높은 구간을 식별한다"고 결론내릴 수는 없었다.

시간 순서도 중요하다. **D7에서 먼저 확인된 것은 익일 수익률 산포의 차이였고, 그 차이가 무엇을 의미하는지는 당시 검정되지 않았다.** 이후 이 관측을 출발점으로 D8b에서 사건별 사후 경로변동성을 별도로 정의하고, 직전 실현변동성을 고려한 층화 비교를 수행했다. 따라서 D8b의 결과는 D7에서 이미 발견했다고 주장할 수 있는 결론이 아니라, D7에서 관측된 산포 차이가 어떤 구조를 갖는지 후속으로 검증하기 위해 추가된 분석이다.

이 단계에서 확정할 수 있는 것은 **평균수익률의 유의한 효과가 아니라, S1 신호일의 익일 수익률 산포가 대조군과 크게 달랐다는 관측이 존재했고 그 관측이 최초 검정의 유의성에 중요한 영향을 주었다는 것**이다. 그 산포 차이가 경제적으로 어떤 의미를 갖는지, 또는 기존 변동성 정보 이상의 정보를 제공하는지는 이 시점의 결과만으로 판단하지 않는다.

> §6.1 표의 "사후 변동성" 표현은 익일 수익률 하나의 산포를 가리키며, D8b의 경로변동성과 다르다. 본문 3문단 참조.
