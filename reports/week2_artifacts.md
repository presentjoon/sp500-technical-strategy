...
| **D4** | 08-05 | **어떤 지표가 국면 차이를 통과시키는가?** — 동차 차수 사다리 완성(ATR 원값 6.05배 → RSI 1.25배). H3′ 지지(구조가 지표 계열보다 중요), **H2는 역전**(볼린저 1.31배 vs RSI 7.80배) |
...
| `reports/day04_phase_indicator_stats.csv` | D4 | `e0d71aa` | **"동차 차수가 국면 간 배율을 결정한다"** — ATR 원값 6.05배 ~ RSI 1.25배 사다리. 1차 동차 지표를 시기 간 비교에 쓰면 안 되는 이유 | `day02_phase_stats.csv` | 유효 |
